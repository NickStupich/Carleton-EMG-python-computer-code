...
def dtwAcceptPortionOfInputs(lx, ly, distanceFunc):
	...
	for x in range(lenX):
		costs[x][0] = IGNORING_PENALTY_FUNC(float(x) / lenX) + distanceFunc(lx[x], ly[0])
	for y in range(lenY):
		costs[0][y] = IGNORING_PENALTY_FUNC(float(y) / lenY) + distanceFunc(lx[0], ly[y])
		
	costs[0][0] = distanceFunc(lx[0], ly[0])
	
	for x in range(1, lenX):
		for y in range(1, lenY):
			costs[x][y] = distanceFunc(lx[x], ly[y])
			
	#add the ignoring penalty on the 'exit', last row/column
	for x in range(lenX):
		costs[x][-1] += IGNORING_PENALTY_FUNC((lenX - float(x)-1) / lenX)
	for y in range(lenY):
		costs[-1][y] += IGNORING_PENALTY_FUNC((lenY - float(y)-1) / lenY)
